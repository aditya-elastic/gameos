...
import { readArtifactContent, toProjectRelativeArtifactPath, writeArtifact, writeWorkspaceArtifacts } from "./artifacts";
...
export function recordEngineQa(projectId: string, report: EngineQaReport): ProjectWorkspace {
  const workspace = getStudioProject(projectId);
  if (!workspace) {
    throw new Error(`Project not found: ${projectId}`);
  }

  const label = `${report.lane === "godot" ? "Godot" : "Unity"} Engine QA`;
  const artifact = writeArtifact(
    projectId,
    "engine-qa-report",
    label,
    `${report.lane}-engine-qa-report.md`,
    renderEngineQaMarkdown(workspace, report)
  );
  addArtifact(artifact);

  const updatedWorkspace = getWorkspace(projectId);
  if (!updatedWorkspace) {
    throw new Error(`Project disappeared after ${report.lane} QA recording: ${projectId}`);
  }
...
function renderUnityAdvancedPlaytestMarkdown(workspace: ProjectWorkspace, report: UnityAdvancedPlaytestReport): string {
  return [
    ...
    "",
    "## Verdict",
    `- Agent: ${report.agent ?? "Advanced Player Council - Unity Engine Lane"}`,
    `- Claim: ${report.claim ?? "scene-aware capability engine playtest"}`,
    `- Verdict: ${report.verdict ?? "unknown"}`,
    "",
    "## Scene Gate",
    `- Scene loaded: ${String(Boolean(report.scene_loaded))}`,
    `- Controller found: ${String(Boolean(report.controller_found))}`,
    `- Watermark found: ${String(Boolean(report.watermark_found))}`,
    "",
    "## Player Metrics",
    ...
    "Unity remains promoted as a local capability engine lane when the scene loads, the controller and watermark are present, and the advanced player approves the short validation pacing. Build export and store publishing remain behind later Build Sentinel gates."
  ].join("\n");
}

function renderEngineQaMarkdown(workspace: ProjectWorkspace, report: EngineQaReport): string {
  const capabilityMap = workspace.artifacts.find((artifact) => artifact.kind === "capability-map");
  const acceptanceProfile = workspace.artifacts.find((artifact) => artifact.kind === "acceptance-profile");
  const adapterKind = report.lane === "godot" ? "godot-adapter" : "unity-adapter";
  const adapterArtifact = workspace.artifacts.find((artifact) => artifact.kind === adapterKind);

  return [
    `# ${workspace.project.name} ${report.lane === "godot" ? "Godot" : "Unity"} Engine QA`,
    "",
    "## Verdict",
    `- Result: ${report.ok ? "ENGINE_QA_PASS" : "ENGINE_QA_BLOCKED"}`,
    `- Exit status: ${report.status ?? "not reported"}`,
    `- Error: ${report.error ?? "none"}`,
    "",
    "## Provenance",
    "- Generated by: Game OS",
    "- Watermark required: true",
    "- Watermark label: Made with GameOS",
    "- Boundary: local engine test lane only; no store or platform publishing automation.",
    "",
    "## Evidence Inputs",
    `- Capability map: ${capabilityMap ? toProjectRelativeArtifactPath(capabilityMap.path, workspace.project.id) : "missing"}`,
    `- Acceptance profile: ${acceptanceProfile ? toProjectRelativeArtifactPath(acceptanceProfile.path, workspace.project.id) : "missing"}`,
    `- Adapter artifact: ${adapterArtifact ? toProjectRelativeArtifactPath(adapterArtifact.path, workspace.project.id) : "missing"}`,
    "",
    "## Command",
    "```bash",
    report.command.map((part) => (part.includes(" ") ? JSON.stringify(part) : part)).join(" "),
    "```",
    "",
    "## Project Root",
    report.projectRoot,
    ...
    "## Stdout",
    "```text",
    (report.stdout ?? "").trim() || "none",
    "```",
    "",
    "## Stderr",
    "```text",
    (report.stderr ?? "").trim() || "none",
    "```",
    "",
    "## Architecture Decision",
    report.ok
      ? "The engine adapter produced runnable local QA evidence tied to the capability map, acceptance profile, watermark policy, and Game OS provenance."
      : "The engine adapter is blocked until the failed command is fixed and rerun with the same capability and acceptance evidence."
  ].join("\n");
}
