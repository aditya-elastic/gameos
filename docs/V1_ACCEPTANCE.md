...
- Web games require browser visual QA screenshots, readable play surface, no horizontal overflow, watermark, controls, browser interaction proof, first-10-seconds, replay, control feel, clarity, difficulty curve, visual maturity, and Advanced Player Council gates for `WORTH_PLAYING`. v0.7 browser-proves arcade survival, platform movement, and combat survival as reusable capability patterns, not named game lanes. Asset-led Web games also require no-goal-magnet physics, timing skill, player agency, mastery, smooth primary gesture input, smooth mouse/touch blade input, slow human mouse blade input, input/reset, and asset-fit gates.
...
- `npm run acceptance:capability-web-quality` creates arcade survival, platform movement, and combat survival fixtures, runs browser QA, runs trust review, and requires `CREATOR_TEST_READY` for those three capability patterns.
