...
assert(existsSync(path.join(projectRoot, "Assets", "Editor", "GameOsUnityPlayerAgent.cs")), `Missing Unity player agent at ${projectRoot}`);
assert(unityEditor, "Unity editor not found. Set UNITY_EDITOR=/path/to/Unity or install Unity through Unity Hub.");

runUnity(["-batchmode", "-nographics", "-quit", "-projectPath", projectRoot, "-executeMethod", "GameOS.Editor.GameOsUnityPlayerAgent.Run", "-logFile", "-"]);
...
function runUnity(args) {
  ...
  assert(result.status === 0, "Unity player agent verdict was not worth playing. Upgrade architecture before accepting this prototype.");
}
